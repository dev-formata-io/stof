...
use crate::{SGraph, Library, SNum, SNumType, SType, SVal};
use super::Object;
...
impl Library for StringLibrary {
    ...
    fn call(&mut self, graph: &mut SGraph, name: &str, parameters: &mut Vec<SVal>) -> Result<SVal> {
        if parameters.len() > 0 {
            let this = &parameters[0].cast(SType::String)?;
            match this {
                SVal::String(val) => {
                    match name {
                        "len" => {
                        ...
                        "at" => {
                            if parameters.len() == 2 {
                                let second = &parameters[1].cast(SType::Number(SNumType::I64))?;
                                match second {
                                    SVal::Number(nval) => {
                                        let index = nval.int();
                        ...
                        "concat" => {
                            if parameters.len() == 3 {
                                ...
                                match second {
                                    SVal::String(second) => {
                                        let third = &parameters[2].cast(SType::String)?;
                                        match third {
                                            SVal::String(third) => {
                        ...
                        "substring" => {
                            if parameters.len() == 2 {
                                ...
                                match second {
                                    SVal::Number(start) => {
                                        let third = &parameters[2].cast(SType::Number(SNumType::I64))?;
                                        match third {
                                            SVal::Number(end) => {
