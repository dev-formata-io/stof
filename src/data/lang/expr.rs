...
impl Expr {
    ...
    /// Engine exec.
    pub async fn engine_exec(&self, pid: &str, doc: &Arc<Mutex<SDoc>>) -> Result<SVal> {
        match self {
            Expr::Variable(_) => {
            ...
            Expr::Tuple(vals) => {
                let mut vec: Vec<SVal> = Vec::new();
                for val in vals {
                    vec.push(Box::pin(val.engine_exec(pid, doc)).await?);
                }
                Ok(SVal::Tuple(vec))
            },
            Expr::Array(vals) => {
            ...
            Expr::NewObject(statements) => {
                ...
                // Make sure to set new object as self_ptr for new sub-objects!
                let init_statements = Statements::from(init_statements);
                let _ = Box::pin(init_statements.engine_exec(pid, doc)).await;
                
                doc.lock().await.pop_self(pid);
                return Ok(SVal::Object(stof_object));
            },
            Expr::Cast(stype, expr) => {
                let value = Box::pin(expr.engine_exec(pid, doc)).await?;
                ...
                return Ok(value.cast(target, pid, &mut doc)?);
            },
            Expr::TypeOf(expr) => {
                let value = Box::pin(expr.engine_exec(pid, doc)).await?;
            ...
            Expr::TypeName(expr) => {
                let value = Box::pin(expr.engine_exec(pid, doc)).await?;
                Ok(SVal::String(value.type_name(&doc.lock().await.graph)))
            },
            Expr::Not(expr) => {
                let value = Box::pin(expr.engine_exec(pid, doc)).await?;
                Ok(SVal::Bool(!value.truthy()))
            ...
            Expr::Call { scope, name, params } => {
                ...
                if let Some(lib) = doc.lock().await.library(&library_name) {
                    ...
                    match &variable_value {
                        SVal::Object(nref) => {
                            ...
                            if let Some(func) = SFunc::func(&doc.lock().await.graph, name, '.', Some(&nref)) {
                                ...
                                let res = Box::pin(func.engine_call(pid, doc, func_params, true)).await?;
                                ...
                                if let Some(prototype) = doc.lock().await.graph.node_ref(&prototype_field.to_string(), None) {
                                    ...
                                    while current.is_some() {
                                        if let Some(func) = SFunc::func(&doc.lock().await.graph, &func_name, '.', current.as_ref()) {
                                            ...
                                            let res = Box::pin(func.engine_call(pid, doc, func_params, false)).await?;
                                            {
                                                let mut doc = doc.lock().await;
                                                doc.pop_self(pid);
                                                doc.set_table(pid, current_symbol_table);
                    ...
                    let mut library = lib.write().unwrap();
                    let res = Box::pin(library.engine_call(pid, doc.clone(), name, &mut func_params)).await?;
                ...
                // If here, scope is not a field, func, object, or symbol
                // Check to see if scope is a library itself before falling back to std lib
                if let Some(lib) = doc.lock().await.library(&scope) {
                    let mut func_params = Vec::new();
                    for expr in params {
                        let val = Box::pin(expr.engine_exec(pid, doc)).await?;
                        if !val.is_void() {
