...
/// Markdown variation of the TEXT format
pub struct MD;
impl MD {
    /// Parse some md into a new graph.
    pub fn parse(text: &str) -> SGraph {
        let mut graph = SGraph::default();
        let root = graph.insert_root("root");
        SField::new_string(&mut graph, "markdown", text, &root);
        graph
    }

    /// Stringify.
    pub fn stringify(pid: &str, doc: &SDoc) -> Result<String, SError> {
        if let Some(field) = SField::field(&doc.graph, "markdown", '.', doc.graph.main_root().as_ref()) {
            return Ok(field.to_string());
        }
        Err(SError::fmt(pid, &doc, "markdown", "did not find a 'markdown' field on the main root of this graph to stringify"))
    }

    /// Stringify node.
    pub fn stringify_node(pid: &str, doc: &SDoc, node: impl IntoNodeRef) -> Result<String, SError> {
        if let Some(field) = SField::field(&doc.graph, "markdown", '.', Some(&node.node_ref())) {
            return Ok(field.to_string());
        }
        Err(SError::fmt(pid, doc, "markdown", "did not find a 'markdown' field on this node to stringify"))
    }
}
impl Format for MD {
    /// Format getter.
    ...
    fn header_import(&self, pid: &str, doc: &mut crate::SDoc, _content_type: &str, bytes: &mut bytes::Bytes, as_name: &str) -> Result<(), SError> {
        ...
        match res {
            ...
            Err(error) => {
                Err(SError::fmt(pid, &doc, "markdown", &error.to_string()))
            }
        }
    }
    ...
    fn export_string(&self, pid: &str, doc: &crate::SDoc, node: Option<&crate::SNodeRef>) -> Result<String, SError> {
        if node.is_some() {
            Self::stringify_node(pid, &doc, node)
        } else {
            Self::stringify(pid, &doc)
        }
    }
}
