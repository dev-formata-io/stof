...
impl PKG {
    ...
    pub fn create_package_zip(dir_path: &str, dest_path: &str, excluded: &HashSet<String>) -> Option<String> {
        ...
        let res = PKG::zip_directory(&mut iter.filter_map(|e| e.ok()), dir_path, file, zip::CompressionMethod::Bzip2, excluded);
        if res.is_err() {
            return None;
        }
        return Some(path);
...
impl Format for PKG {
    ...
    fn file_import(&self, pid: &str, doc: &mut SDoc, _format: &str, full_path: &str, extension: &str, as_name: &str) -> Result<(), SError> {
        ...
        if extension == "zip" {
            ...
            created_temp = true;
            import_path.push_str(".stof"); // just because of the below logic (importing a dir instead of a file)
        }
